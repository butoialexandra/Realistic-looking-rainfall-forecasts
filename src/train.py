...
def train_loop_conditional(opt, netG, netD, optimizerG, optimizerD, criterion, fixed_noise, train_ds, valid_ds):
    global_step = 0
    if opt.add_l1:
    ...
    for epoch in range(opt.epochs):
        for i, (x, y) in enumerate(train_ds, 0):
            ...
            if i == 0:  # len(dataloader) - 1:
                ...
                # plot_images(fake.detach().cpu().numpy(), f"{opt.outf}/fake_samples_epoch_{epoch}.png")
                plot_image_single_conditional(fake[0][0].detach().cpu().numpy(), real_cpu[0][0].cpu().numpy(),
                                              f"{opt.outf}/image_pair_{epoch}.png")
                plot_images_ncols(
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', required=False, default='cgan', help='gan | cgan | unet')
    ...
    parser.add_argument('--conditional', default=True, action='store_true', help='flag to indicate if using cluster')
    parser.add_argument('--highres', default=True, action='store_true', help='flag to indicate if using cluster')
    ...
    parser.add_argument('--add-skip-connections', default=True, action='store_true', help='whether to use skip connections in GAN, as in pix2pix')
    ...
    if opt.model in ['gan', 'cgan']:
        ...
        if opt.dry_run:
            opt.epochs = 1
        if opt.model == 'cgan':
            train_loop_conditional(opt, netG, netD, optimizerG, optimizerD, criterion, fixed_noise, train_dataloader, valid_dataloader)
        else: # opt.model == 'gan':
